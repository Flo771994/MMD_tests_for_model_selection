...
# Change the directory in R 
ST.ERROR <- round(seq(1.2, 3, by=0.1), digits=1)
...
for(i.sd.new in 1:5 )
{
  ...
  load(paste("Worspace-Fig-4-SD-of-Y-is-", ST.ERROR[i.sd.new],".RData", sep=""))
...
for (iii in 1:length(DIM))
{
  pdf(paste("2-models-NONDEGEN-Fig4-dim-", DIM[iii], ".pdf", sep =""))
  plot(ST.ERROR[1:5], res.SW[iii, 1,  ],  pch=20, type="b",  
       lwd=2, col=1,  main= paste("p=", DIM[iii], sep=""), 
       ylim=c(0,1),xlim=c(1.2,1.9),          xlab="Stand. dev.", ylab="Emp. Level/Power")
  for(i in 1: length(SAMPLE.SIZE))
  {
    if( i!=1 ) 
    {
    ...
    legend(1.7, 1, leg.txt, pch=c(20, 21), col=rep(5,2))
    
    leg.txt <- c("n=100","n=250", "n=500", "n=1000") 
    legend(1.7, 0.5, leg.txt, lty=c(1,2,3,4), col=c(1,2,3,4),  lwd=c(2,2,2,2))
