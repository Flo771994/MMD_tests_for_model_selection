...
for (i.sd in 1:length(ST.ERROR))
{
  ...
  # THETA1 = Dimension, JD suggestion
  THETA1 = DIM
  ...
  print(iii)
    ...
    for(ijk in 1:length(SAMPLE.SIZE))
    {
      ...
      # Export R-functions
      clusterExport(cl, "Ex.Funct.Fig.4")
      clusterExport(cl, "fun.parallel.Fig.4")
      ...
      # Parallelization!
      outT <- foreach(x = 1:NK) %dopar% {fun.parallel.Fig.4(x)}
...
save.image(paste("Worspace-Fig-4-SD-of-Y-is-", SD.temp, ".RData",sep="" ))
