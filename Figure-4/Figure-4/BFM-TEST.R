...
SIM.sig.fig.2 = function(d, n, MU.temp= 0)
{
  # matrices of dimension nxd
  X = matrix(rnorm(n*d, mean=0), ncol = d)
  # 
  Y = matrix(rnorm(n*d, mean=0), ncol = d)
  
  # Estimate sd using X-data
  hat.sd.X = apply(X, 2, sd)
  
  # Matrix, each row is hat.mu
  matr.hat.sd =  matrix(hat.sd.X, ncol=d, nrow=n, byrow = T)
  # Y * SD matrix, each column is multiplied by marginal SD
  
  hat.Y  = matrix(MU.temp, ncol=d, nrow=n, byrow = T) + Y*matr.hat.sd
  
  out = list("X"=X, "Y"=Y, "hat.Y"=hat.Y, "hat.sd.X"=hat.sd.X )
  return(out)
}
...
##### FIGURE 1
Ex.Funct.Fig.1 <- function(NR=NR,  n=n, d=d, theta1=theta1, 
                           epsilon=epsilon, SD.temp=SD.temp)
{ # Beginning of the function
  
  spec.test.stat   <- array(0, c(NR, length(epsilon)))
  spec.test.stat.q <- array(0, c(NR))
  
  spec.reject      <- array(0, c(NR, length(epsilon)))
  spec.reject.q    <- array(0, c(NR))
  ...
  {  
    ...
    spec.test.stat[ijk, ] = res$spec.test.stat.1
    spec.reject[ijk, ]    = res$spec.reject.1
    
    
    spec.test.stat.q[ijk] = res$spec.test.stat.q.1
    spec.reject.q[ijk]    = res$spec.reject.q.1
    
  }
  
  
  out <- list("spec.test.stat"= spec.test.stat, 
              "spec.test.stat.q"=spec.test.stat.q, 
              #
              "spec.reject"= spec.reject,
              "spec.reject.q"=spec.reject.q
              )
...
# Argument is "x" is the number of the kernel
# SEED is a global variable
fun.parallel.Fig.1 <- function(x)
{
  set.seed(SEED[x])
  out <- Ex.Funct.Fig.1(NR=NR,  n=n, d=d, theta1=theta1, 
                        epsilon=epsilon, SD.temp=SD.temp)
  return(out)
}
...
Ex.Funct.Fig.2 <- function(NR=NR,  n=n, d=d, theta1=theta1, 
                           epsilon=epsilon, MU.temp=MU.temp)
{ # Beginning of the function
...
# Argument is "x" is the number of the kernel
# SEED is a global variable
fun.parallel.Fig.2 <- function(x)
{
  set.seed(SEED[x])
  out <- Ex.Funct.Fig.2(NR=NR,  n=n, d=d, theta1=theta1, 
                        epsilon=epsilon, MU.temp=MU.temp)
  return(out)
}
...
Ex.Funct.Fig.3 <- function(NR=NR,  n=n, d=d, theta1=theta1, 
                           epsilon=epsilon, 
                           SD.theor1 = SD.theor1, 
                            SD.theor2=SD.theor2)
{ # Beginning of the function
  
  selec.test.stat   <- array(0, c(NR, length(epsilon)))
  selec.test.stat.q <- array(0, c(NR))
  
  selec.reject      <- array(0, c(NR, length(epsilon)))
  selec.reject.q    <- array(0, c(NR))
  
  for (ijk in 1:NR)
  {  
    
    dat    = SIM.mu.2sampl(d, n, SD.theor1=SD.theor1, SD.theor2=SD.theor2)
    X      = dat$X
    hat.Y1 = dat$hat.Y1
    hat.Y2 = dat$hat.Y2
    ...
    selec.test.stat[ijk, ] = res$selec.test.stat
    selec.reject[ijk, ]    = res$selec.reject
    
    
    selec.test.stat.q[ijk] = res$selec.test.stat.q
    selec.reject.q[ijk]    = res$selec.reject.q
...
# Argument is "x" is the number of the kernel
# SEED is a global variable
fun.parallel.Fig.3 <- function(x)
{
  set.seed(SEED[x])
  out <- Ex.Funct.Fig.3(NR=NR,  n=n, d=d, theta1=theta1, 
                        epsilon=epsilon, 
                        SD.theor1=c(1,1), SD.theor2=c(1,SD.temp))
  return(out)
}
...
Ex.Funct.Fig.4 <- function(NR=NR,  n=n, d=d, theta1=theta1, 
                           epsilon=epsilon, 
                           SD.theor1 = SD.theor1, 
                           SD.theor2=SD.theor2)
{ # Beginning of the function
...
# Argument is "x" is the number of the kernel
# SEED is a global variable
fun.parallel.Fig.4 <- function(x)
{
  set.seed(SEED[x])
  out <- Ex.Funct.Fig.4(NR=NR,  n=n, d=d, theta1=theta1, 
                        epsilon=epsilon, 
                        SD.theor1=c(1.2,1.2), SD.theor2=c(1.2,SD.temp))
  return(out)
}
