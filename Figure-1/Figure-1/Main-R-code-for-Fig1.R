...
for (i.sd in 1:length(ST.ERROR))
{
  ...
  print(iii)
    ...
    for(ijk in 1:length(SAMPLE.SIZE))
    {
      ...
      # Export R-functions
      clusterExport(cl, "Ex.Funct.Fig.1")
      clusterExport(cl, "fun.parallel.Fig.1")
      ...
      # Parallelization!
      outT <- foreach(x = 1:NK) %dopar% {fun.parallel.Fig.1(x)}
...
save.image(paste("Worspace-MP-0-SD-of-Y-is-", SD.temp, ".RData",sep="" ))
